...
ai_review/
└── user_stories/
    ├── booking-creation.yaml
    ├── booking-status.yaml
    ├── payment-quote-flow.yaml
    ├── payment-fees.yaml
    └── *.yaml             # Additional story files
screenshots/
└── bowser-qa/
    └── 20260210_143022_a1b2c3/        # Run directory (datetime + short uuid)
        ├── booking-creation/          # Source file stem
        │   ├── admin-creates-standard-booking/   # Slugified story name
...
8. For each story, build its `SCREENSHOT_PATH` by combining three parts:
   - `RUN_DIR` (from step 7)
   - Source file stem (filename without `.yaml` extension, e.g. `booking-creation.yaml` → `booking-creation`)
   - Slugified story name (lowercase, replace spaces with hyphens, e.g. `"Admin creates standard booking"` → `admin-creates-standard-booking`)

   Concatenate: `SCREENSHOT_PATH = "{RUN_DIR}/{file-stem}/{slugified-name}/"`

   Example: `screenshots/bowser-qa/20260210_143022_a1b2c3/booking-creation/admin-creates-standard-booking/`
...
11. For each story, spawn a `bowser-qa-agent` teammate via the Task tool with `team_name: "ui-review"` and `mode: "bypassPermissions"`. Launch ALL teammates in a single message so they run in parallel.
